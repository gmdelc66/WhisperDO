...
# -------------------------------------------------------------------------------
#
# Descripción del Script, genera cascadas de caracteres en la terminal,
#   simulando un efecto visual similar al de la película "The Matrix".
#   Con las funcionalidades adicionales como escuchar audio, convertirlo en texto
#   y monitorear archivos en una carpeta,
#   Necesitamos librerias adicionales como watchdog.
#
# Aquí te doy una guía de cómo podrías integrar estas funcionalidades:
#
#     Escuchar y convertir audio a texto: Usaremos la biblioteca speech_recognition para capturar el audio
#     y convertirlo en texto.
#     Monitorear una carpeta: Utilizaremos la biblioteca watchdog para monitorear la carpeta
#     "leer" y ejecutar una acción cuando se detecte un nuevo archivo.
#     Texto en pantalla: Modificaremos el código original para que cambie el color de las
#     letras cuando detecte voz, y para que muestre el texto convertido.
#
##############################################################################################################
...
class FileHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        print(f"Texto detectado por Whisper: {result['text']}")


def listen_to_audio(recognizer, mic):
    try:
        with mic as source:
        ...
        filename = f"escucha/texto_{time.strftime('%Y%m%d%H%M%S')}.txt"
        ...
        print(f"No se pudieron obtener los resultados del servicio de reconocimiento de voz; {e}")
    return ""
...
def main():
    ...
    mic = sr.Microphone()
    whisper_model = whisper.load_model("base")  # Carga el modelo de Whisper
    ...
    observer = Observer()
    observer.schedule(FileHandler(whisper_model), path="leer", recursive=False)
    observer.start()

    cascading = set()
    while True:
        text = listen_to_audio(recognizer, mic)
